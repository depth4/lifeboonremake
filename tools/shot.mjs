 ...
 *   внутри <назначение> [сцена]  — встать ВНУТРИ здания и смотреть на улицу
 */
const ГЛАЗ = 1.65;
if (args[0] === 'перед' || args[0] === 'вдоль' || args[0] === 'внутри') {
  ...
  if (args[0] === 'внутри') {
    /**
     * Камера внутри здания, у дальней стены, лицом к улице. Это ответ на
     * вопрос Алекса «видно ли улицу изнутри»: стена двусторонняя, окно —
     * дырка, значит видно должно быть само собой. Проверяем, а не верим.
     */
    // от входа вглубь: человек, вошедший в дверь, стоит в проходе
    const вглубьЗала = о.глубина / 2 - 3.5;
    const кx = дx - о.нx * вглубьЗала;
    const кz = дz - о.нz * вглубьЗала;
    const цx = кx + о.нx * 60;
    const цz = кz + о.нz * 60;
    aim.from = `${кx.toFixed(1)},${(подНогами(кx, кz) + ГЛАЗ).toFixed(2)},${кz.toFixed(1)}`;
    aim.at = `${цx.toFixed(1)},${(подНогами(цx, цz) + 1.8).toFixed(2)},${цz.toFixed(1)}`;
  } else if (args[0] === 'перед') {
    aim.from = `${кx.toFixed(1)},${(подНогами(кx, кz) + ГЛАЗ).toFixed(2)},${кz.toFixed(1)}`;
