...
const checks = [
  ...
  ['удар на живой странице', true,
    crash.count > 0
      ...
      : 'догонял, но не задел'],
  /**
   * Что проверяется здесь и чего здесь проверять НЕЛЬЗЯ.
   *
   * Стояло «город назвал выезд на встречную». Утверждение это про то,
   * что СЛУЧИЛОСЬ за сорок пять секунд живой езды: машина выезжает
   * на встречную там, где её оставила предыдущая часть поездки, а где
   * она её оставит — зависит от всего города сразу. 17 сентября город
   * переставили (дома стали разного размера), и в тот же миг на том же
   * месте оказался светофор: город честно записал проезд на красный,
   * а встречной в этот раз не было. Проверка упала на том, что стало
   * ЛУЧШЕ, — а это не проверка, это лотерея.
   *
   * Сам по себе выезд на встречную проверяется детерминированно и без
   * браузера: `npm run traffic`, «выезд игрока на встречную замечен —
   * 1 раз, а не за каждый кадр». Браузеру остаётся то, чего нет у
   * терминала: что вся цепочка доходит до ЖИВОЙ СТРАНИЦЫ — нарушение
   * замечено, названо словами и доехало до приборки.
   */
  ['город заметил нарушение и назвал его', offences.length > 0
    && offences.every((o) => typeof o.what === 'string' && o.what.length > 0),
