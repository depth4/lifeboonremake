...
/**
 * Действие на странице, которое может зависнуть вместе с браузером.
 *
 * В контейнере нет видеокарты, и WebGL иногда сваливается на программный
 * отрисовщик прямо посреди прогона. Страница на секунды перестаёт отвечать,
 * и `page.mouse.move` падает по своему сроку. Раньше это роняло ВЕСЬ прогон
 * с необработанной ошибкой: приговор не печатался, остальные пятнадцать
 * проверок пропадали, и в выводе оставалась куча сообщений драйвера, из
 * которых причина не читается.
 *
 * Теперь такое действие записывается как проблема — проверка от этого не
 * слабеет: проблема всё равно валит прогон, но валит ПОНЯТНО, и видно,
 * что успело пройти до неё.
 */
const тихо = async (что, действие) => {
  try { await действие(); } catch (e) {
    problems.push(`${что}: ${String(e).split('\n')[0]}`);
  }
};
...
await тихо('выворот руля вправо', async () => {
 ...
 */
/**
 * Двадцать секунд ЖИЗНИ МИРА, а не сорок пять.
 *
 * Сорок пять я взял из прежнего числа, не переведя единицы: раньше это были
 * сорок пять секунд ЧАСОВ, то есть около десяти секунд мира. Сорок пять
 * секунд мира на программном отрисовщике — это три минуты по часам, и весь
 * прогон `npm test` вырастал вдвое. Двадцати хватает с запасом: машина
 * на четырнадцати метрах в секунду проходит за это время без малого триста
 * метров, а квартал у нас двести двадцать.
 */
const МИРОВЫХ_СЕКУНД = 20;
...
for (let tick = 0; tick < 900 && crash.count <= ударовБыло && мирПрожил < МИРОВЫХ_СЕКУНД; tick++) {
  ...
  if (now.car !== null && Math.hypot(now.car.x, now.car.z) > 300) {
    await page.keyboard.up('w');
    await тихо('разворот на краю квартала', async () => {
      for (let i = 0; i < 12; i++) await page.mouse.move(800 + i * 90, 500);
      await page.waitForTimeout(2500);
      for (let i = 0; i < 12; i++) await page.mouse.move(800 - i * 90, 500);
    });
 ...
 */
await тихо('перенос на встречную', async () => {
  for (let i = 0; i < 5; i++) await page.mouse.move(800 - i * 90, 500);
});
await мир(2.6);
await тихо('возврат в свою полосу', async () => {
  for (let i = 0; i < 5; i++) await page.mouse.move(800 + i * 90, 500);
});
await мир(1.5);
await page.keyboard.up('w');
...
await тихо('выйти из машины', () => page.keyboard.press('Enter'));
await page.waitForTimeout(400);
 ...
 *
 * Каждое действие здесь — через `тихо`, и вот почему. Шаг стоит СРАЗУ ПОСЛЕ
 * снимка, а снимок — это первое, что перестаёт получаться, когда браузер
 * подвисает на программном отрисовщике. Прогон, в котором снимок «ride-11-пдд»
 * не вышел, дальше упирался в голый `page.click`: тот ждал свои тридцать
 * секунд и падал необработанной ошибкой, унося ВЕСЬ приговор — пятнадцать
 * проверок, которые уже прошли, не печатались вовсе. Ровно та беда, ради
 * которой `тихо` и написано; сюда его просто забыли поставить.
 */
await тихо('поставить машину', () => page.click('button[data-drive="park"]'));
await тихо('сесть обратно', () => page.click('button[data-drive="seat"]'));
await page.waitForTimeout(500);
await тихо('взять руль', () => page.mouse.click(800, 500));
await тихо('пересесть в салон', () => page.keyboard.press('c'));
await тихо('газ', () => page.keyboard.down('w'));
await мир(2.5);
await тихо('отпустить газ', () => page.keyboard.up('w'));
await мир(0.3);
