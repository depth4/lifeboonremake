...
export function* полеПоШагам(
  ...
  if (наЗемле !== null && как.занято !== false) {
    // большой дом — десятки тысяч клеток: передышки и посреди него, по строкам
    const закрыть = function* (часть: НаЗемле, мх0: number, мх1: number, мz0: number, мz1: number): Generator<void, void, void> {
    ...
    for (const д of наЗемле.дома) {
      ...
      yield* закрыть({ ...пусто, дома: [д] }, д.x - hx, д.x + hx, д.z - hz, д.z + hz);
    }
