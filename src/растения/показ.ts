...
export interface Трава {
  ...
  источник(строить: ((окно: { x0: number; z0: number; ширина: number; глубина: number }) => Поле) | null): void;
...
export function создатьТраву(сцена: THREE.Scene, солнце: THREE.DirectionalLight): Трава {
  ...
  let строить: ((окно: { x0: number; z0: number; ширина: number; глубина: number }) => Поле) | null = null;
  ...
  return {
    ...
    кадр(камера, время) {
      ...
      // поле — окно вокруг камеры; ушли от его середины — строим новое
      if (поле === null || Math.hypot(cx - серединаX, cz - серединаZ) > ОТХОД) {
        const x0 = Math.floor((cx - ОКНО / 2) / КЛЕТКА) * КЛЕТКА;
        const z0 = Math.floor((cz - ОКНО / 2) / КЛЕТКА) * КЛЕТКА;
        поставитьПоле(строить({ x0, z0, ширина: ОКНО, глубина: ОКНО }));
        серединаX = cx; серединаZ = cz;
      }
