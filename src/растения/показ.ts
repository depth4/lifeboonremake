...
/**
 * Цвет травинки, линейный — корень, верхушка и сухое пятно. Отсюда берёт
 * цвет и шейдер травинки, и газон под травой (`ГАЗОН`).
 */
const ТРАВИНКА = {
  корень: [0.018, 0.04, 0.008],
  верх: [0.15, 0.29, 0.05],
  сухая: [0.3, 0.25, 0.09],
} as const;
/** Какая доля пути от корня к верхушке видна издали: низ травинки закрыт соседками. */
const ВИДНО_ИЗДАЛИ = 1;
/**
 * ЦВЕТ ГАЗОНА — трава, какой её видно издали, линейный.
 *
 * До 25.09 у земли был свой цвет (0x5f8a4a), у травинки — свой, и они
 * разошлись: земля светлее и голубее. Вдали травинки редеют, земля между
 * ними просвечивает — и редкая трава читалась правильными точками на
 * светлом, а её край — чертой (Алекс: «дальности травы недостаточно»).
 * Так делают все, кто рисует траву: под травой земля её цвета, и где
 * травинок уже нет, глаз этого не отличает. Теперь цвет один.
 */
export const ГАЗОН: readonly [number, number, number] = [0, 1, 2].map((i) =>
  ТРАВИНКА.корень[i] + (ТРАВИНКА.верх[i] - ТРАВИНКА.корень[i]) * ВИДНО_ИЗДАЛИ) as [number, number, number];
const вGLSL = (c: readonly number[]): string => `vec3(${c.map((v) => v.toFixed(4)).join(', ')})`;

/** До скольких метров от глаза видны травинки. Дальше — газон её цвета. */
const ДАЛЬ = 64;
...
const ТЕЛО = /* glsl */`
  ...
  if (!alive) {
    ...
    // реже — шире: до шестикратной ширины, дальше просвет закрывает газон её цвета
    float W = uWidth * (0.75 + 0.5 * r3) * clamp(inversesqrt(max(keep, 0.02)), 1.0, 6.0);
    ...
    // нормаль: к камере, скруглена поперёк, вдали — НОРМАЛЬ САМОЙ ЗЕМЛИ под
    // травинкой, из тех же высот поля. До 25.09 вдали она смотрела в небо
    // (0, 1, 0): на склоне к солнцу земля светлела, а травинки нет, и редкая
    // трава читалась тёмными точками на светлом
    vec2 hu = (fc + 0.5) / (uFieldCells + 1.0);
    vec2 hs = 4.0 / (uFieldCells + 1.0);
    float hx = texture(uHeights, hu + vec2(hs.x, 0.0)).r - texture(uHeights, hu - vec2(hs.x, 0.0)).r;
    float hz = texture(uHeights, hu + vec2(0.0, hs.y)).r - texture(uHeights, hu - vec2(0.0, hs.y)).r;
    vec3 groundN = normalize(vec3(-hx, 8.0 * uFieldCell, -hz));
    ...
    objectNormal = normalize(mix(nrm, groundN, 0.3 + 0.5 * smoothstep(6.0, uFar, dist)));

    // цвет: тёмный корень, светлая верхушка, пятна суше и сочнее
    vec3 root = ${вGLSL(ТРАВИНКА.корень)};
    vec3 tipC = ${вGLSL(ТРАВИНКА.верх)} * (0.8 + 0.4 * r3);
    tipC = mix(tipC, ${вGLSL(ТРАВИНКА.сухая)}, smoothstep(0.55, 0.85, patchD) * 0.75);
    vec3 col = mix(root, tipC, pow(sp, 0.7));
    col *= mix(1.0, mix(0.6, 1.0, sp), 1.0 - smoothstep(10.0, uFar, dist));
    // вдали травинка редкая и видна целиком, от тёмного корня: у настоящей
    // корень закрыт соседками. Поэтому к краю дальности она целиком цвета
    // верхушки — того же, что газон под ней, и край травы не читается чертой
    col = mix(col, tipC, smoothstep(10.0, uFar * 0.72, dist));
    vGrass = col;
...
/**
 * Насколько камера может отойти от середины поля и сторона окна поля, м.
 * Окно ВЫВОДИТСЯ из дальности травы: травинке на краю дальности, пока
 * камера отходит, нужна земля под ней. Отодвинуть край травы и забыть про
 * окно теперь нельзя — трава обрывалась бы на краю поля, а не дальности.
 */
const ОТХОД = 32;
const ОКНО = 2 * (ДАЛЬ + ОТХОД);
