...
/**
 * Предел во дворе, км/ч.
 *
 * ПДД 10.2: в жилых зонах и на дворовых территориях не более 20.
 *
 * Поставить это число удалось не сразу. С ним машина, въезжающая со скорости
 * улицы, оказывалась на +59 к знаку двора — не потому, что гоняет, а потому
 * что тормозить ей было негде: желаемая скорость менялась в миг смены дороги,
 * а настоящая падала метров через полсотни. Двадцатка работает вместе
 * с заглядыванием на знак следующей дороги, и порознь они бессмысленны.
 */
export const ДВОР_LIMIT = 20;
...
export function buildSigns(
  ...
  const limit = world.shapes.map((_, si) => {
    /**
     * ВО ДВОРЕ — ДВАДЦАТЬ. ПДД 10.2: в жилых зонах и на дворовых территориях
     * не более 20 км/ч. Это не украшение: 21.09, когда проезд во дворе стал
     * длинным и прямым, машина разогналась в нём до 91 км/ч. Двор от улицы
     * отличается тем же, чем и всегда, — отсутствием осевой.
     */
    if (!улица(world, si)) return ДВОР_LIMIT;
