...
export interface Mover {
  ...
  хозяин: number;
  /**
   * До какого часа хозяин будет внутри, когда доедет, — в часах ОТ НАЧАЛА
   * ЖИЗНИ ГОРОДА, а не по кругу суток: 8 — сегодня в восемь, 32 — завтра
   * в восемь, Infinity — сегодня на ней не поедут. `null` — ехать ему
   * некуда (за город или ничья), значит и вставать незачем.
   */
  доЧаса: number | null;
  /**
  ...
  /**
   * Место в кармане. `доЧаса` — час (от начала жизни города), когда хозяин
   * выйдет; до него машина стоит. Раньше здесь был обратный отсчёт
   * `8 + случайно × 25` секунд, то есть стоянка без причины.
   */
...
export interface Bay {
  ...
  /** Ширина полосы стоянки, м. По ней же считается, встала машина или нет. */
...
/**
 * Насколько машина имеет право промахнуться мимо середины кармана.
 *
 * Не допуск «на глаз», а РОВНО то, что остаётся от полосы стоянки после
 * кузова. Пока здесь стояло 0.6 м, машина «вставала», торча из стоянки
 * на 30 см в полосу движения: замерено 17 раз за два минуты на «городе».
 * Теперь «встал, но торчит» невыразимо — либо кузов в стоянке целиком,
 * либо машина ещё едет.
 */
const промах = (bay: Bay): number => Math.max(0.05, bay.ширина / 2 - WIDE);
...
export function buildNetwork(
  world: World,
  /**
   * `стоянка: 'в полосе движения'` — вернуть карманы туда, где они были
  ...
  /**
   * Карманы. Место, где машина стоит, — это ПОЛОСА СТОЯНКИ, и никакая другая.
   *
   * До 17.09 карманом объявлялась правая полоса движения: отдельного места
   * для стоянки на наших дорогах не было вовсе. Из этого одного решения
   * росло всё остальное — стоящая машина перегораживала полосу, её надо
   * было объезжать по встречной, объезд приносил столкновения (решение 079),
   * а ряд припаркованных запирал сам себя, и выезжать приходилось сразу
   * в соседнюю полосу.
   *
   * Теперь карман ВЫВОДИТСЯ из полосы с `kind: 'parking'`. Дорога без такой
   * полосы карманов не имеет вовсе, и «машина стоит там, где ездят»
   * записать негде: полоса стоянки по построению не полоса движения
   * (`lanes.ts` берёт только `travel`).
   *
   * У перекрёстков карманов по-прежнему нет — там переходы и обзор.
   */
  const bays: Bay[] = [];
  const lanes = buildLanes(world);
  world.shapes.forEach((shape, si) => {
    const полосыСтоянки = как.стоянка === 'в полосе движения'
      ? [...lanes[si].forward.slice(0, 1), ...lanes[si].backward.slice(0, 1)]
        .map((l) => ({ across: l.across, ширина: l.width }))
      : bands(shape.type)
        .filter((b) => b.kind === 'parking')
        .map((b) => ({ across: (b.from + b.to) / 2, ширина: b.to - b.from }));
    if (полосыСтоянки.length === 0) return;
    const total = length[si];
     ...
     * и 4.6 на заезд.
     */
    const шаг = КВАРТАЛ.место.длина + 3.7;
    for (let at = edge; at < total - edge; at += шаг) {
      if (nodes[si].some((n) => Math.abs(n.s - at) < shape.outerHalf + 6)) continue;
      for (const п of полосыСтоянки)
        bays.push({ shape: si, s: at, across: п.across, ширина: п.ширина, taken: -1 });
    }
  });
...
export function moveTraffic(
  ...
  options: {
    ...
    /**
     * Час ОТ НАЧАЛА ЖИЗНИ ГОРОДА (не по кругу суток: после полуночи — 24,
  ...
  movers.forEach((m, index) => {
    ...
    if (rules && ahead !== null && mine !== null && ahead.stopGap < 70) {
       ...
       *
       * Обе попытки были, пока стоянкой служила правая полоса движения.
       * С 17.09 карман — отдельная полоса, и машина, СТОЯЩАЯ в кармане,
       * дорогу не занимает вовсе: затором она не бывает. Затор — въезжающая
       * и выезжающая (они наполовину в полосе) и разбитая. 25.09, когда
       * утром на улицу вышли все, однополосный проезд двора с карманами
       * вдоль него не пропускал через перекрёсток никого.
       */
    ...
    if (m.park !== null) {
      ...
      if (m.park.phase === 'въезжает') {
         ...
         */
        if (gap < ДОЕХАЛ && m.speed < 0.6 && Math.abs(m.across - bay.across) <= промах(bay)) {
          m.park.phase = 'стоит';
          m.speed = 0;
        }
        ...
        /**
         * «Пора» — просто «уже не раньше». До 25.09 часы сравнивались по кругу
         * суток с окном в двенадцать часов: машина, вставшая у дома в 18:30
         * с выездом завтра в 8, в 19:00 считала, что «пора», и уезжала без
         * хозяина. Час от начала жизни города по кругу не ходит.
         */
        const пора = час >= m.park.доЧаса;
        if (пора) {
          /**
           * Выезжает В СВОЮ ПРАВУЮ ПОЛОСУ — ту самую, по которой приехал.
           *
           * До 17.09 приходилось выезжать через одну: карманом была правая
           * полоса, впереди в ней в шести метрах стояла следующая
           * припаркованная машина, и ряд запирал сам себя (из 149 машин
           * утром уезжали 17). Стоянка отдельной полосой это убрала —
           * впереди по полосе движения не стоит никто, потому что стоять
           * там негде.
           */
          m.lane = 0;
        ...
        if (Math.abs(m.across - lane) < 0.15) { net.bays[m.park.bay].taken = -1; m.park = null; }
          ...
          /**
           * Полоса занята — ВОЗВРАЩАЕМСЯ В СТОЯНКУ, а не замираем поперёк.
           *
           * Раньше машина просто останавливалась там, где её застало
           * «занято», — то есть наполовину в полосе движения. Дальше беда
           * складывалась сама: подъехавшему сзади не проехать, значит полоса
           * не освобождается, значит выезжающий не трогается, значит она
           * не освободится никогда. Поймано 18.09 на «городе»: во дворе
           * встало кольцо из шестнадцати машин, двое стояли по семьдесят
           * шесть секунд.
           *
           * Отдельного правила это не заводит: то же самое уже сделано
           * для перестроения — «у машины всегда есть куда деваться».
           * «Стою поперёк полосы и жду» перестало быть выразимым.
           */
          const назад = net.bays[m.park.bay].across;
          m.speed = 0;
          m.reason = 'выезжает, ждёт';
          m.across += Math.max(-0.9 * dt, Math.min(0.9 * dt, назад - m.across));
          return;
        } else {
