...
function wantLane(world: World, net: Network, movers: readonly Mover[], m: Mover): number {
  ...
  /** Есть ли такая полоса и свободна ли она. Своя считается свободной всегда. */
  const свободна = (to: number): boolean =>
    to >= 0 && to <= left && (to === m.lane || laneSafe(movers, m, line(to)));
   ...
   */
  return свободна(хочет) ? хочет : m.lane;
}
