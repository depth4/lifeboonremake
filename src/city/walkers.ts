...
export interface Walker {
  ...
  /**
   * Которая по счёту дорога дня жителя это: зачем он идёт, берётся из его
   * распорядка по этому номеру, а не копируется сюда. У ничьего −1.
   */
  дорога: number;
  /**
   * Кого ведёт за руку — номера дошкольников. Своего пешехода у ребёнка
   * на этой дороге нет: он там, где рука взрослого (`заРуку`), и потеряться
   * ему нечем.
   */
...
/**
 * Где пешеход стоит и куда смотрит. Говорящий смотрит на того, с кем
 * говорит: взгляд ВЫЧИСЛЯЕТСЯ из места собеседника и не хранится.
 */
export function walkerPose(world: World, w: Walker): { x: number; z: number; yaw: number } {
  ...
  if (w.разговор !== null) {
    ...
    return { x: точка.x, z: точка.z, yaw: Math.atan2(там.z - точка.z, там.x - точка.x) };
  }
...
/**
 * Где идёт ребёнок, которого ведут за руку: сбоку от взрослого, лицом туда
 * же. Первый — с ДАЛЬНЕЙ от проезжей части стороны: так детей и водят;
 * второй — за другую руку. Сторона берётся от САМОЙ УЛИЦЫ (наружу от оси
 * тротуара), а не от того, куда взрослый смотрит: первая редакция ставила
 * ребёнка справа по взгляду, и взрослый, обернувшийся к догнавшему его
 * знакомому, переставлял ребёнка к дороге. На переходе — сбоку вдоль улицы.
