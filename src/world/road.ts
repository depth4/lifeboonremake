 ...
 * Что это за полоса.
 *
 * `parking` — СТОЯНКА: асфальт, по которому не ездят. Добавлена 17.09
 * ради одного: пока стоянки не было, стоящая машина занимала полосу
 * движения, и «объехать стоящего по встречной» становилось обязательным
 * механизмом. Отдельная полоса делает это состояние невыразимым —
 * стоящей машине негде оказаться на пути у едущей.
 *
 * Ни одна существующая дорога её не получает: `roadTypeForLanes` про неё
 * не знает. Кто хочет стоянку — складывает свой тип дороги сам.
 */
export type LaneKind = 'travel' | 'marking' | 'median' | 'parking';
