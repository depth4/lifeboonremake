...
import { ШАГ, type Walker, moveWalkers, placeWalkers, walkerPose, заРуку, фазаШага } from './city/walkers.ts';
import {
  СЕКУНД_В_ЧАСЕ, type Занятие, type Зачем, type Квартира, type Расселение, болеет, где as гдеЖителя, деньНедели, кто, провожатыйКвартиры, расселить,
} from './city/житель.ts';
import {
  ЧАС_УТРА, type Снаружи, вывестиНаУлицу, гдеЖитель, госномер, дворНаЧас, жить, окраскаМашины, одеждаЖителя,
  ростЖителя, своиМеста, сколькоМашин, сколькоПешеходов,
} from './city/жизнь.ts';
...
function зеленьСцены(): void {
  ...
  viewer.setВещи(н.вещи.map((вещь) => ({ вещь, низ: полВещи(вещь) })));
  вещиДворов = н.вещи;
  /**
...
/** Вещи дворов — те же, что отданы показу в зелени: на них сидят и у них играют. */
let вещиДворов: readonly Вещь[] = [];
/**
 * Пол вещи — самая высокая земля под её следом (`площадкаПодСледом`, как
 * у дома): коробка 26×14 на неровной земле, поставленная по одной точке
 * своей середины, пробивалась землёй полосами. Раз на вещь и на землю.
 */
const полВещей = new WeakMap<Вещь, { земля: GroundIndex; пол: number }>();
function полВещи(в: Вещь): number {
  const было = полВещей.get(в);
  if (было !== undefined && было.земля === ground) return было.пол;
  const пол = площадкаПодСледом({ x: в.x, z: в.z, курс: в.курс, ширина: в.ширина, глубина: в.длина },
    (x, z) => ground.sample(x, z).height, 0).верхЗемли;
  полВещей.set(в, { земля: ground, пол });
  return пол;
}
/** На чём стоит дворовый: на полу вещи (коробка, лавка) или на земле. */
const подНогами = (на: Вещь | null, x: number, z: number): number =>
  на === null ? ground.sample(x, z).height : полВещи(на) + (на.что === 'коробка' ? 0.04 : 0);
/**
...
панельСлежки.style.cssText = 'position:fixed;left:16px;bottom:96px;max-width:340px;padding:10px 13px;'
  ...
  + 'font:12px/1.5 var(--mono);white-space:pre-wrap;display:none;pointer-events:none;z-index:5';
...
const ЗАЧЕМ: Record<Зачем, string> = {
  ...
  врач: 'к врачу', двор: 'во двор', прогулка: 'погулять', гости: 'в гости к родне', дача: 'на дачу', дом: 'домой',
};
const КТО: Record<string, string> = { сад: 'дошкольник', школа: 'школьник', работа: 'работает', дома: 'пенсионер' };
const КРУГ: Record<Круг, string> = {
  семья: 'из своей семьи', сосед: 'сосед по дому', коллега: 'коллега', одноклассник: 'одноклассник', 'по саду': 'из одной группы сада', приятель: 'приятель',
};
...
/** Болеет ли он сам и не дома ли с больным ребёнком — из того же расчёта, что и его день. */
function здоровьеСловами(р: Расселение, номер: number, занятие: Занятие, квартира: Квартира, сутки: number): string[] {
  const своя = болеет(р, номер, занятие, сутки);
  if (своя !== null) return [`болеет: ${сутки - своя.с + 1}-й день из ${своя.до - своя.с}`];
  if (провожатыйКвартиры(квартира) !== номер) return [];
  const больные = квартира.кто.flatMap((з, i) => (з === 'сад' && болеет(р, квартира.первый + i, 'сад', сутки) !== null
    ? [`№${квартира.первый + i}`] : []));
  return больные.length === 0 ? [] : [`дома с больным ребёнком: ${больные.join(', ')}`];
}
...
function деньСловами(номер: number, час: number, точка: ReturnType<typeof гдеЖитель>): string {
  ...
  const сутки = Math.floor(час / 24);
  const ж = кто(жизнь, номер, сутки);
  ...
  // встречи суток — из того же журнала, на который смотрят сами говорящие
  const встречи = (снаружи?.разговоры ?? []).filter((р2) => р2.a.житель === номер || р2.b.житель === номер);
  const идёт = встречи.find((р2) => час < р2.до);
  const сКем = (р2: (typeof встречи)[number]): string =>
    `№${р2.a.житель === номер ? р2.b.житель : р2.a.житель}, ${КРУГ[р2.круг]}`;
  const строки = [
    `Житель №${номер} · ${КТО[ж.занятие] ?? ж.занятие}`,
    `живёт: ${здание(ж.дом)}`,
    ...здоровьеСловами(жизнь, ж.номер, ж.занятие, ж.квартира, сутки),
    ...(ж.заРулём ? [`машина: ${окраскаМашины(номер).имя}, ${госномер(номер, жизнь.посёлок.сид)}; `
      + `${своиМеста(world, network, жизнь).has(номер) ? 'у дома своё место' : 'у дома места нет — ставит во дворе'}`] : []),
    `сейчас ${деньНедели(сутки)}, ${чч(час)}: ${идёт !== undefined ? `встретил знакомого — говорит (${сКем(идёт)})` : сейчас}`
      + `${точка.как === 'внутри' || точка.как === 'во дворе' ? ' — ждём у входа' : ''}`,
    '',
    ...ж.день.map((д, i) => `${м.где === 'в пути' && м.i === i ? '▶' : ' '} ${чч(д.выход)}–${чч(д.приход)}  ${ЗАЧЕМ[д.зачем]}`
      ...
      + `${д.ведёт.includes(номер) ? `, за руку с №${провожатыйКвартиры(ж.квартира)}`
        : д.ведёт.length > 0 ? `, ведёт ${д.ведёт.map((n) => `№${n}`).join(', ')}` : ''}`),
    ...(встречи.length === 0 ? [] : ['', 'встречи сегодня:',
      ...встречи.map((р2) => `  ${чч(р2.от)}  ${сКем(р2)}, ${Math.max(1, Math.round((р2.до - р2.от) * 60))} мин`)]),
  ];
  return строки.join('\n');
}
...
function заселить(): void {
  ...
  if (посёлок === null) {
    ...
    снаружи = вывестиНаУлицу(world, network, жизнь, часЖизни(), () => вещиДворов);
...
viewer.onFrame((dt) => {
  ...
  if (городЖив) {
    ...
    // во дворах вокруг камеры — те, кто по распорядку во дворе: на лавках, у горок, в коробке
    const глаз = viewer.глаз();
    const сид = жизнь?.посёлок.сид ?? 0;
    const воДворе = снаружи === null ? []
      : дворНаЧас(снаружи, часЖизни(), { x: глаз.x, z: глаз.z, r: 180 }).map((д) => ({
        x: д.x, y: подНогами(д.на, д.x, д.z), z: д.z, yaw: д.yaw,
        ...одеждаЖителя(д.номер, сид),
        фаза: (д.прошёл / (2 * ШАГ * д.рост)) * 2 * Math.PI, сидит: д.сидит, рост: д.рост,
      }));
    // кого ведут за руку — сбоку от взрослого, шагом своего роста
    const заРукой = walkers.flatMap((w) => w.ведёт.map((ребёнок, k) => {
      const п = заРуку(world, w, k);
      const рост = ростЖителя('сад');
      return {
        x: п.x, y: ground.sample(п.x, п.z).height, z: п.z, yaw: п.yaw,
        ...одеждаЖителя(ребёнок, сид), фаза: (w.путь / (2 * ШАГ * рост)) * 2 * Math.PI, рост,
      };
    }));
    viewer.setWalkers([...walkers.map((w) => {
      const pose = walkerPose(world, w);
      /**
       * Говорят по очереди: каждые три секунды слово переходит к другому,
       * и жестикулирует тот, чья очередь. Очередь — из часов и места в паре,
       * не хранится.
       */
      const р2 = w.разговор;
      const слово = р2 !== null && (Math.floor(cityTime / 3) + (р2.a === w ? 0 : 1)) % 2 === 0;
      return {
        x: pose.x, y: ground.sample(pose.x, pose.z).height, z: pose.z, yaw: pose.yaw,
        colour: w.colour, штаны: w.штаны, кожа: w.кожа, фаза: фазаШага(w), рост: w.рост,
        ...(слово ? { жест: cityTime * 4.5 + (w.seed % 7) } : {}),
      };
    }), ...заРукой, ...воДворе]);
...
(window as unknown as { __разговоры?: () => unknown }).__разговоры = () => (снаружи?.разговоры ?? [])
  .filter((р2) => часЖизни() < р2.до)
  .map((р2) => {
    const a = walkerPose(world, р2.a), b = walkerPose(world, р2.b);
    return { a: р2.a.житель, b: р2.b.житель, круг: р2.круг, ax: a.x, az: a.z, bx: b.x, bz: b.z, до: р2.до };
  });
