...
import {
  СЕКУНД_В_ЧАСЕ, type Зачем, type Расселение, где as гдеЖителя, кто, провожатыйКвартиры, расселить,
} from './city/житель.ts';
import {
...
function деньСловами(номер: number, час: number, точка: ReturnType<typeof гдеЖитель>): string {
  ...
  const строки = [
    ...
    `сейчас ${чч(час)}: ${идёт !== undefined ? `встретил знакомого — говорит (${сКем(идёт)})` : сейчас}`
