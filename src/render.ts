...
export function show(
  ...
  return {
    ...
    setBuildings(дома) {
      ...
      застройка = [цоколь, стены, плоские, скаты, окна, двери, рамы, входы];
      scene.add(...застройка);
      ...
      дома.forEach(({ дом: д, площадка: пл }, i) => {
        ...
        /**
         * КУДА САДИТСЯ ПРОЁМ — одно место на все четыре грани.
         *
         * Раньше здесь стоял вопрос «на фасаде или нет», и третьего ответа
         * не было. Дворовая грань (22.09) в такую развилку не помещается:
         * она смотрит туда же, куда фасад, только в другую сторону.
         * Грань — это знак и ось, и больше про неё знать нечего.
         *
         * Доворот — НАРУЖУ от грани: местная ось x проёма смотрит из дома.
         * Коробке стекла всё равно, куда она смотрит, а раме нет: до 24.09
         * доворот у двора и у левого торца смотрел внутрь дома, и рама
         * там ушла бы в стену.
         */
        const куда = (п: { грань: number; вдоль: number }): {
