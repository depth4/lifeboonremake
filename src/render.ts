    ...
    setBuildings(дома) {
      ...
      дома.forEach(({ дом: д, площадка: пл }, i) => {
        ...
        const куда = (п: { грань: number; вдоль: number }): {
          ...
          ? { вперёд: (п.грань === 0 ? 1 : -1) * (д.глубина / 2), вбок: п.вдоль, доворот: 0 }
          : { вперёд: п.вдоль, вбок: (п.грань * д.ширина) / 2, доворот: Math.PI / 2 });

        for (const п of д.двери) {
          const м = куда(п);
          часть(двери, дверейN++, д, пл.пол, м.вперёд, ДВЕРЬ.высота / 2, м.вбок,
            ТОЛЩИНА, ДВЕРЬ.высота, ДВЕРЬ.ширина, м.доворот, 0x3a2f28);
        }
        for (const п of д.окна) {
          const м = куда(п);
          часть(окна, оконN++, д, пл.пол, м.вперёд,
            п.этаж * ЭТАЖ + ОКНО.отПола + ОКНО.высота / 2, м.вбок,
