...
export interface Viewer {
   ...
   */
  setСтоянка(cars: Parameters<Viewer['setTraffic']>[0]): void;
  /** Трафик: положения едущих машин, каждый кадр. Пустой список — убрать всех. */
  ...
  setWalkers(people: readonly {
    ...
    /** Сидит (на лавке): корпус на высоте сиденья, ноги вперёд. */
    сидит?: boolean;
    /** Рост в долях взрослого: вся фигура в этом масштабе. */
    рост?: number;
    /** Говорит: фаза жеста, радианы. Нет — руки висят или машут в такт шагу. */
    жест?: number;
  }[]): void;
  /** Где глаз камеры: показу знать, какие дворы оживлять рядом. */
  глаз(): { x: number; z: number };
...
export function show(
  ...
  /**
   * ПАРТИЯ МАШИН — кузова, колёса и огни пачками. Их две: едущие
   * перерисовываются каждый кадр, стоящие в кармане — только когда список
   * стоящих поменялся. До 25.09 все машины города шли одной партией каждый
   * кадр, и на «большом городе» с его тысячей стоящих это съедало кадр.
   *
   * Партия растёт запасом (вдвое), а не пересоздаётся при каждом изменении
   * числа: при улице по распорядку машины выходят и уходят постоянно,
   * и каждое пересоздание — новые буферы на видеокарте.
   */
  type Машина = Parameters<Viewer['setTraffic']>[0][number];
  const партияМашин = (): ((cars: readonly Машина[]) => void) => {
    let body: THREE.InstancedMesh | null = null;
    let wheels: THREE.InstancedMesh | null = null;
    let lamps: THREE.InstancedMesh | null = null;
    let запас = 0;
    const m = new THREE.Matrix4(), wm = new THREE.Matrix4(), lm = new THREE.Matrix4();
    ...
    const углы: [number, number][] = [[2.1, 0.72], [2.1, -0.72], [-2.1, 0.72], [-2.1, -0.72]];
    return (cars) => {
      if (cars.length > запас && body !== null) {
        scene.remove(body, wheels as THREE.Object3D, lamps as THREE.Object3D);
        body.dispose(); wheels?.dispose(); lamps?.dispose();
        body = null; wheels = null; lamps = null;
      }
      if (body === null) {
        if (cars.length === 0) return;
        запас = Math.max(64, cars.length * 2);
        ...
        wheels = new THREE.InstancedMesh(wheel, wheelMaterial, запас * 4);
        /**
        ...
        lamps = new THREE.InstancedMesh(lamp, new THREE.MeshBasicMaterial(), запас * 4);
        scene.add(body, wheels, lamps);
      }
      const b = body, w = wheels as THREE.InstancedMesh, l = lamps as THREE.InstancedMesh;
      b.count = cars.length; w.count = cars.length * 4; l.count = cars.length * 4;
      cars.forEach((c, i) => {
        m.makeRotationY(-c.yaw);
        m.setPosition(c.x, c.y, c.z);
        b.setMatrixAt(i, m);
        b.setColorAt(i, tint.setHex(c.colour));
        const cos = Math.cos(c.yaw), sin = Math.sin(c.yaw);
        corners.forEach(([ahead, left], k) => {
          wm.makeRotationY(-c.yaw);
          wm.setPosition(c.x + ahead * cos - left * sin, c.y + 0.33, c.z + ahead * sin + left * cos);
          w.setMatrixAt(i * 4 + k, wm);
        });
        /**
         ...
         */
        углы.forEach(([ahead, left], k) => {
          lm.makeRotationY(-c.yaw);
          lm.setPosition(c.x + ahead * cos - left * sin, c.y + 0.62, c.z + ahead * sin + left * cos);
          l.setMatrixAt(i * 4 + k, lm);
          ...
          l.setColorAt(i * 4 + k, tint.setHex(lit));
        });
      });
  ...
  const едущие = партияМашин();
  const стоящие = партияМашин();
  ...
  /**
   * На сколько человек собраны пачки. Растут вдвое, а не пересобираются
   * при каждом изменении числа: улица по распорядку меняет его почти
   * каждый кадр, и каждое пересоздание — новые буферы на видеокарте.
   */
  let запасЛюдей = 0;
  /**
   ...
   * а мерить надо не их. `__кадр(true)` включает картинку обратно: город
   * живёт в настоящем времени без неё, а снимок делается, когда пора.
   */
  let безКадра = new URLSearchParams(location.search).get('кадр') === 'нет';
  (window as unknown as { __кадр?: (рисовать: boolean) => void }).__кадр = (рисовать) => { безКадра = !рисовать; };
  const рисовать = (): void => {
    кадрТравы();
    if (безКадра) return;
  ...
  return {
    ...
    setTraffic(cars) { едущие(cars); },
    setСтоянка(cars) { стоящие(cars); },
    setSignals(lamps) {
    ...
    глаз() { return { x: camera.position.x, z: camera.position.z }; },
    setWalkers(people) {
      if (люди !== null && people.length > запасЛюдей) {
        for (const часть of Object.values(люди)) { scene.remove(часть); часть.dispose(); }
        люди = null;
      }
      if (люди === null) {
        if (people.length === 0) return;
        запасЛюдей = Math.max(64, people.length * 2);
        люди = собратьЛюдей(запасЛюдей);
        scene.add(люди.тело, люди.голова, люди.лицо, люди.ноги, люди.руки);
      }
      люди.тело.count = люди.голова.count = люди.лицо.count = people.length;
      люди.ноги.count = люди.руки.count = people.length * 2;

      const m = new THREE.Matrix4();
      const q = new THREE.Quaternion();
      const e = new THREE.Euler();
      const тон = new THREE.Color();
      const масштаб = new THREE.Vector3(1, 1, 1);
      const где = new THREE.Vector3();
      let k = 1;
      ...
      const часть = (
        ...
        // рост — одним множителем на всю фигуру: и места частей, и сами части
        const a = вперёд * k, b = вверх * k, c = вбок * k;
        // местное «вперёд» — это (cos, sin) по земле, «вбок» — (−sin, cos)
        где.set(p.x + a * cos - c * sin, p.y + b, p.z + a * sin + c * cos);
        // сначала мах вокруг Z (вперёд-назад), потом разворот по курсу
        e.set(0, -p.yaw, мах, 'YZX');
        q.setFromEuler(e);
        m.compose(где, q, масштаб);
        меш.setMatrixAt(номер, m);
        меш.setColorAt(номер, тон.setHex(цвет));
      };
      ...
      people.forEach((p, i) => {
        ...
        /**
         * Сидит: бедро опускается до сиденья лавки (0.45) и корпус вместе
         * с ним; ноги вытянуты вперёд почти горизонтально, руки — на коленях.
         * Это та же фигура, только другая поза, а не второй человек.
         */
        const вниз = p.сидит === true ? БЕДРО - 0.47 : 0;
        // корпус качается в такт: за один шаг — два покачивания
        const качка = p.сидит === true ? 0 : Math.cos(p.фаза * 2) * 0.012;

        часть(люди!.тело, i, p, 0, БЕДРО + качка - вниз, 0, 0, p.colour);
        часть(люди!.голова, i, p, 0, ГОЛОВА.низ + качка - вниз, 0, 0, p.кожа);
        // лицо выступает на волос из передней грани, иначе грани мерцают друг сквозь друга
        часть(люди!.лицо, i, p, ГОЛОВА.ребро / 2, ЛИЦО.надЗемлёй - ЛИЦО.высота / 2 + качка - вниз, 0, 0, 0x2a2420);
        for (const бок of [0, 1]) {
          const знак = бок === 0 ? 1 : -1;
          const нога = p.сидит === true ? 1.45 : знак * мах;
          // говорящий поднимает правую руку вперёд и водит ею; левая почти висит
          const рука = p.сидит === true ? 0.45
            : p.жест !== undefined ? (бок === 0 ? 0.7 + Math.sin(p.жест) * 0.3 : 0.1 + Math.sin(p.жест * 0.6) * 0.06)
              : знак * махРук;
          часть(люди!.ноги, i * 2 + бок, p, 0, БЕДРО + качка - вниз, знак * РАССТАВ, нога, p.штаны);
          часть(люди!.руки, i * 2 + бок, p, 0, ПЛЕЧО + качка - вниз, знак * ПЛЕЧИ, рука, p.colour);
        }
      });
